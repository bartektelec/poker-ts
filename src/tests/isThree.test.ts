...
import isThree from "../utils/isThree.ts";

import type { TRank } from "../consts.ts";

const buildCard = (x: TRank) => new Card(x, SUITS.Clubs);
Deno.test("isThree should return true for three of a kind hands", () => {
  const hands: TRank[][] = [
    ["A", "2", "A", "4", "5", "A", "K"],
    ["6", "5", "8", "5", "10", "3", "5"],
    ["K", "A", "J", "K", "10", "K", "9"],
  ...
  const cardHands = hands.map((hand) => isThree(hand.map(buildCard)));
  const failedCardHands = failHands.map((hand) => isThree(hand.map(buildCard)));
